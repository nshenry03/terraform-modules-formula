...
We will see a quick setup to configure the ``terraform-modules`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``terraform-modules`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``terraform-modules`` formulas under ``/srv/salt/terraform-modules/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/terraform-modules/parameters/dns:domain/
    ln -s dns:domain /srv/salt/terraform-modules/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/terraform-modules/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/terraform-modules/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``terraform-modules`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``terraform-modules`` formulas under ``/srv/salt/terraform-modules/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/terraform-modules/parameters/roles

We will define 2 roles:

- ``terraform-modules/server``
- ``terraform-modules/client``

We create a configuration for the role ``terraform-modules/server`` in ``/srv/salt/terraform-modules/parameters/roles/terraform-modules/server.yaml``:
...
We create another configuration for the role ``terraform-modules/client`` in ``/srv/salt/terraform-modules/parameters/roles/terraform-modules/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles terraform-modules/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles terraform-modules/client
...
.. code-block:: console
    ...
    mv /srv/salt/terraform-modules/parameters/dns:domain/  '/srv/salt/terraform-modules/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    {%- from tplroot ~ "/map.jinja" import mapdata as terraform-modules with context %}

    test-does-nothing-but-display-terraform-modules-as-json:
      test.nop:
        - name: {{ terraform-modules | json }}
...
Here is an example based on `template-formula/terraform-modules/config/file.sls`_:
...
.. code-block:: sls
    ...
    terraform-modules-config-file-file-managed:
      file.managed:
        ...
        - mode: 644
        - user: root
        - group: {{ terraform-modules.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
          - sls: {{ sls_package_install }}
        - context:
            terraform-modules: {{ terraform-modules | json }}

This ``sls`` file expose a ``terraform-modules`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ terraform-modules['winner'] }}
...
.. _template-formula/terraform-modules/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/terraform-modules/config/file.sls
